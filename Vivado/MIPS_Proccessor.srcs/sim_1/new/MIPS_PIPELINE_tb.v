...
module MIPS_PIPELINE_tb;
    ...
    // INSTRUCTION DECODE
    wire ID_EX_controlEX_RegDst;        // controlEX[3] (RegDst)
    wire [1:0] ID_EX_controlEX_ALUOp;   // controlEX[2:1] (ALUOp[1:0])
    wire ID_EX_controlEX_ALUSrc;        // controlEX[0] (ALUSrc)
    wire [2:0] ID_EX_controlMEM;        // [Branch, MemRead, MemWrite]
    ...
    // EXECUTE
    wire EX_MEM_controlMEM_Branch;       // controlMEM[2]
    wire EX_MEM_controlMEM_MemRead;      // controlMEM[1]
    wire EX_MEM_controlMEM_MemWrite;     // controlMEM[0]
    wire [1:0] EX_MEM_controlWB;         // [RegWrite, MemToReg]
    ...
    wire MEM_WB_controlWB_RegWrite;      // controlWB[1]
    wire MEM_WB_controlWB_MemToReg;      // controlWB[0]
    ...
    wire [31:0] memory0,   memory1,   memory2,   memory3,   memory4,   memory5,   memory6,   memory7,
    ...
    MIPS_PIPELINE MIPS_PIPELINE_INST(
        .clk(clk),
        .rst(rst),
        ...
        // INSTRUCTION DECODE
        .ID_EX_controlEX_RegDst(ID_EX_controlEX_RegDst),
        .ID_EX_controlEX_ALUOp(ID_EX_controlEX_ALUOp),
        .ID_EX_controlEX_ALUSrc(ID_EX_controlEX_ALUSrc),
        .ID_EX_controlMEM(ID_EX_controlMEM),
        .ID_EX_controlWB(ID_EX_controlWB),
        ...
        //  EXECUTE
        .EX_MEM_controlMEM_Branch(EX_MEM_controlMEM_Branch),
        .EX_MEM_controlMEM_MemRead(EX_MEM_controlMEM_MemRead),
        .EX_MEM_controlMEM_MemWrite(EX_MEM_controlMEM_MemWrite),
        .EX_MEM_controlWB(EX_MEM_controlWB),
    ...
    begin
        clk = 0;
        forever #10 clk = ~clk;  // 5 time units
    end
